...
rule flair:
    input:
        expand("results/flair/collapse/{tissue}.isoforms.bed", tissue=util.get_tissues(config)),
        expand("results/flair/collapse/{tissue}.isoforms.gtf", tissue=util.get_tissues(config)),
        expand("results/flair/collapse/{tissue}.isoforms.fa", tissue=util.get_tissues(config)),


rule bam_to_bed12:
    input:
        bam=lambda wildcards: util.get_alignment_for_sample(config, wildcards.sample),
    output:
...
rule combine_bed12:
    input:
        bed12=lambda wildcards: expand(
            ...
            sample=util.get_samples_for_tissue(config, wildcards.tissue),
        ),
    output:
...
rule flair_correct:
    ...
    threads: 8
    shell:
        "flair correct --query {input.bed12} --genome {params.genome_fa} --gtf {input.gtf} --threads {threads} --output {params.output_prefix} > {log} 2>&1"
...
rule flair_collapse:
    input:
        ...
        reads=lambda wildcards: expand(
            ...
            experiment=util.get_experiments_for_tissue(config, wildcards.tissue),
        ),
    output:
    ...
    threads: 8
    shell:
        "flair collapse --query {input.bed12} --genome {params.genome_fa} --reads {input.reads} --gtf {input.gtf} --threads {threads} --output {params.output_prefix} > {log} 2>&1"
